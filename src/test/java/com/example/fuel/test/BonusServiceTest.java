...
class BonusServiceTest {
    ...
    void testBonusDeductionWhenPayingWithBonus() {
        ...
        // Положим в корзину услугу ID=3 (Кофе, цена=70)
        cartService.addToCart(new com.example.fuel.model.CartItem(1, "SERVICE", 3, 1.0));
        double sum = cartService.calculateCartTotal(1, productService);
        assertEquals(70.0, sum, 0.001);

        // Попробуем оплатить бонусами (у клиента бонусов 150, суммы 70 хватает)
        ...
        // После списания бонусов должно остаться 150 - 70 = 80
        Customer after = customerService.getCustomer(1);
        assertEquals(80.0, after.getBonusPoints(), 0.001);
